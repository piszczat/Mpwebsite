...
function Contact({ locale }: { locale: Locale }) {
  const t = copy[locale]; const [ready, setReady] = useState<boolean | null>(null); const [state, setState] = useState<"idle" | "sending" | "success" | "error">("idle"); const [startedAt] = useState(Date.now());
...
export function PortfolioApp({ page }: { page: PageId }) {
  ...
  useEffect(() => { const saved = localStorage.getItem("marcinp-locale"); if (saved === "pl" || saved === "en") setLocale(saved); if (page === "home") { const q = new URLSearchParams(location.search).get("view"); if (q === "about" || q === "projects" || q === "contact") setView(q); } }, [page]);
  ...
  const goHome = (x: HomeView) => { setPaletteOpen(false); if (page === "home") { setView(x); history.replaceState({}, "", `/?view=${x}`); } else location.href = `/?view=${x}`; };
  const go = (path: string) => { setPaletteOpen(false); location.href = path; };
